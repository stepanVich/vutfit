...
**/
  ...
  *RootPtr = malloc(sizeof(struct tBTNode)); 
  ...
  // Strom neni prazdny
  // Zacneme vyhledavat vhodne misto pro vlozeni prvku
  tBTNodePtr refUzel = (*RootPtr); // Ukazatel na aktualni uzel
  while(1) {
   if(refUzel->Cont == Content) break;
   if(Content < refUzel->Cont) {
    if(refUzel->LPtr == NULL) {
     // Provedeme vlozeni do leveho ukazatele
     refUzel->LPtr = malloc(sizeof(struct tBTNode));
     refUzel->LPtr->Cont = Content;
     refUzel->LPtr->LPtr = NULL;
     refUzel->LPtr->RPtr = NULL;
     // Ukoncime smycku
     return;
    } else {
     // Prejdeme do leveho uzlu
     refUzel = refUzel->LPtr;
    }
   } else {
    if(refUzel->RPtr == NULL) {
     // Provedeme vlozeni do praveho ukazatele
     refUzel->RPtr = malloc(sizeof(struct tBTNode));
     refUzel->RPtr->Cont = Content;
     refUzel->RPtr->LPtr = NULL;
     refUzel->RPtr->RPtr = NULL;
     // Ukoncime smycku
     return;
    } else {
     // Prejdeme do praveho uzlu
     refUzel = refUzel->RPtr;
    } 
   }
  } 
 }
...
**/
 ...
 tBTNodePtr uzel = RootPtr;
 while(!SEmptyP(&stack)) {
  uzel = STopPopP(&stack);
  Leftmost_Preorder(uzel->RPtr, &stack);
 }
}
...
**/
 ...
 while( ! SEmptyP(&stack)) {
  uzel = STopPopP(&stack);
  BTWorkOut(uzel);
  Leftmost_Inorder(uzel->RPtr, &stack );
 }
}
...
**/
 ...
 tStackB stackB;
 SInitB(&stackB);
 Leftmost_Postorder(uzel, &stackP, &stackB); 
 while( !SEmptyP(&stackP)) {
  uzel =STopPopP(&stackP);
  SPushP(&stackP, uzel);
  if(STopPopB(&stackB)) {
   SPushB(&stackB, FALSE);
   Leftmost_Postorder(uzel->RPtr, &stackP, &stackB);
  } else {
   STopPopP(&stackP);
   BTWorkOut(uzel);
  }
...
**/
 ...
 SInitP(&stackP);
 while(1) {
  if((*RootPtr)==NULL && !SEmptyP(&stackP)) (*RootPtr)=STopPopP(&stackP);
  if((*RootPtr)!=NULL) {
  ...
  if((*RootPtr) == NULL && SEmptyP(&stackP)) break;
 } 
}
