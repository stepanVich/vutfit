...
void const* gpu_computeVertexAttributeDataPointer(
  ...
  if (head->enabled == 1) {   
    return  (head->offset+(uint8_t*)head->buffer + gl_VertexID*head->stride);   
  }
...
void gpu_runVertexPuller(
  ...
  VertexIndex index_help = gpu_computeGLVertexID(puller->indices, vertexShaderInvocation);    
  for ( size_t i = 0 ; i < MAX_ATTRIBUTES; ++i ){   
    if (puller->heads[i].enabled != 1){  
      output->attributes[i] = NULL; 
    } else {
      output->attributes[i] =  gpu_computeVertexAttributeDataPointer(&puller->heads[i], index_help);            
    }  
  }
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  float
  d00 = dot_Vec2(&vector0, &vector0),   
  d01 = dot_Vec2(&vector0, &vector1),  
  d11 = dot_Vec2(&vector1, &vector1),   
  d20 = dot_Vec2(&vector2, &vector0),   
  d21 = dot_Vec2(&vector2, &vector1),
  jmenovatel =  d00 * d11 - d01 * d01;    
  


  coords->data[1] = (d11 * d20  -  d01 * d21)  /  jmenovatel;    
  coords->data[2] = (d00 * d21  -  d01 * d20)  / jmenovatel;    
